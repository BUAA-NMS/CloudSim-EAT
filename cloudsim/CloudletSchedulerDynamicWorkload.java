...
public class CloudletSchedulerDynamicWorkload extends CloudletSchedulerTimeShared {
	 ...
	 */
	public CloudletSchedulerDynamicWorkload(double mips, int numberOfPes, int ram) {
		super();
		setMips(mips);
	...
	public double getTotalUtilizationOfMem(double time) {
		double ram = 0;
		for (ResCloudlet rcl : getCloudletExecList()) {
			ram += rcl.getCloudlet().getUtilizationOfRam(time);
		}
		return ram;
	}
	/**
...
//		int currentRam = (int)(getTotalUtilizationOfMem(getPreviousTime()) * getRam());
//		return currentRam;
//	}
	public double getCurrentRequestedUtilizationOfMem() {
		// TODO Auto-generated method stub
		return getTotalUtilizationOfMem(getPreviousTime());
	}
	/**
	 * Gets the current mips.
	 * 
...
//		return rcl.getCloudlet().getUtilizationOfCpu(time) * getTotalMips();
	...
	public int getTotalCurrentRequestedRamForCloudlet(ResCloudlet rcl, double time){
//		return (int)rcl.getCloudlet().getUtilizationOfRam(time) * getRam();
		return getRam();
	}
	/**
