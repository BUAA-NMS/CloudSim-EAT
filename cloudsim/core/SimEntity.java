 ...
 * 
 * 
 * SimEntity类负责处理SimEvent事件，并且能够给其他entity发送simEvent
 * 
...
public abstract class SimEntity implements Cloneable {
	...
	/** The buffer for selected incoming events. 为将抵达的event建立缓冲区*/
	 ...
	 * is reserved.
	 * 根据id把事件传给另个entity，Cloudsim负责管理
	 * 
...
//		public static void send(int src, int dest, double delay, int tag, Object data) {
	 ...
	 * <code>9999</code> is reserved.
	 * 调度的优先级最高，在future队列中为future.addEventFirst(e);插入到队列头部
	 * 
	 * @param dest The unique id number of the destination entity
	 * @param delay How long from the current simulation time the event should be sent
	 * @param tag An user-defined number representing the type of event.
	 ...
	 * 抽取在本entity的deferred队列中的匹配模板p的头一个事件
	 * 
	 * @param p The event selection predicate
	 ...
	 * 取消在本entity的deferred队列中的匹配模板p的头一个事件
	 * 
	 * @param p The event selection predicate
	 ...
	 * 从代码看应该不是这个意思，循环获得本entity的event
	 * 
	 ...
	 * deferred queue.
	 * 等待符合模板的event。该方法不检测entity的deferred队列
	 * 
	 * @param p The predicate to match
	 */
	 ...
	 * 每当deferred queue中有事件要求本实体处理时，调用方法
