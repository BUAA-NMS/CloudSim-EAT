 ...
 * all the network mod els from CloudSim, to provide a simpler simulation of networking. In the
...
public class CloudSim {
	...
	/** The simulation clock. 模拟器时间*/
	private static double clock;

	/** Flag for checking if the simulation is running. 当前是否运行*/
	 ...
	 * 添加entity到模拟器中
	 * 这代表了现有模拟器的兼容性，因为实体会被自动添加到仿真实例化后的。
	...
	public static void addEntity(SimEntity e) {
		...
		if (running) {
			...
			// 创建事件evt，SimEvent(int evtype, double time, int src, int dest, int tag, Object edata)
	 ...
	 * 创建event，并加入到future队列中，确定事件的执行事件
	 * 
	 * @param src the src
	 * @param dest the dest
	 * @param delay the delay
