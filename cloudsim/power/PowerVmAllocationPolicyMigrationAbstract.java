...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
	...
	public PowerVmAllocationPolicyMigrationAbstract(
		...
		setFindHostPolicy(Constants.Order);
	}
	/**
...
//		不可用于放置虚拟机的host
	...
	public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		if(getFindHostPolicy()=="BestResource")
			return findHostForVmBestResource(vm, excludedHosts);
		else if (getFindHostPolicy()=="FirstResource")
			return findHostForVmFirstResource(vm, excludedHosts);
		else if (getFindHostPolicy()=="BestPower")
			return findHostForVmBestPower(vm, excludedHosts);
		else if (getFindHostPolicy()=="URS")
			return findHostForVmURS(vm, excludedHosts);
		else System.exit(0);
		return null;
	}
	/**
	...
	public PowerHost findHostForVmBestResource(Vm vm, Set<? extends Host> excludedHosts) {
	//  the first order
			...
			for (PowerHost host : this.<PowerHost> getHostList()) {
				...
				if (host.isSuitableForVm(vm)) {
					...
					double mipsDiff = host.getAvailableMips() - vm.getCurrentRequestedTotalMips();
					if ((mipsDiff < minMips) && (mipsDiff>=0)) {
						minMips = mipsDiff;
						allocatedHost = host;
					}
				}
	...
	public PowerHost findHostForVmFirstResource(Vm vm, Set<? extends Host> excludedHosts) {
	//  the first order
			PowerHost allocatedHost = null;
	...
	public PowerHost findHostForVmURS(Vm vm, Set<? extends Host> excludedHosts) {
		double minPower = Double.MAX_VALUE;
		PowerHost allocatedHost = null;
...
//	protected List< ? extends Host> getUnderUtilizedHostList(Set<? extends Host> excludedHosts) {
//		double minUtilization = 0.1;
//		List <PowerHost> underUtilizedHost = null;
//		for (PowerHost host : this.<PowerHost> getHostList()) {
//			if (excludedHosts.contains(host)) {
//				continue;
//			}
...
//			power = host.getPowerModel().getPower(getMaxUtilizationCpuAfterAllocation(host, vm));
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
		}
