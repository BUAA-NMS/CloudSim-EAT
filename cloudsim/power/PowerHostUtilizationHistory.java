...
/**
 * The class of a host that stores its CPU utilization history and MEM utilization hisory. The history is used by VM allocation
...
public class PowerHostUtilizationHistory extends PowerHost {
	...
	/**
	 * Gets the host utilization CPU history.
	 * 
	 * @return the host utilization CPU history
	 */
	protected double[] getUtilizationCpuHistory() {
	...
	/**
	 * Gets the host utilization Mem history.
	 * 
	 * @return the host utilization Mem history
	 */
	protected double[] getUtilizationMemHistory() {
		double[] utilizationHistory = new double[PowerVm.HISTORY_LENGTH];
		double hostRam = getRam();
		for (PowerVm vm : this.<PowerVm> getVmList()) {
			for (int i = 0; i < vm.getUtilizationHistory().size(); i++) {
				utilizationHistory[i] += vm.getUtilizationHistory().get(i) * vm.getRam() / hostRam;
			}
		}
		return MathUtil.trimZeroTail(utilizationHistory);
	}	
