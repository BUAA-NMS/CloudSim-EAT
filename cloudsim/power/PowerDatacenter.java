			...
			if (!isDisableMigrations()) {
				...
				if (migrationMap != null) {
					for (Map<String, Object> migrate : migrationMap) {
						...
						send(
								getId(),
								(double)vm.getRam() / ((double) targetHost.getBw() / (2 * 8000)),
...
//								migrate);
	 ...
	 * @return the double
	 */
	public double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
		double currentTime = CloudSim.clock();
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
			...
			Log.formatLine(
					"%.2f: [Host #%d] utilization of Cpu is %.2f%%",
					currentTime,
					host.getId(),
					host.getUtilizationOfCpu() * 100);
			Log.formatLine(
					"%.2f: [Host #%d] utilization of Mem is %.2f%%",
					currentTime,
					host.getId(),
					host.getUtilizationOfMem() * 100);			
		}
...
//						timeDiff);
				...
				tmp=timeFrameDatacenterEnergy;

				Log.printLine();
				Log.formatLine(
						"%.2f: [Host #%d] utilization of Cpu at %.2f was %.2f%%, now is %.2f%%" +
						"\n\t\t   utilization of Mem at %.2f was %.2f%%, now is %.2f%%",
						currentTime,
						host.getId(),
						getLastProcessTime(),
						...
						getLastProcessTime(),
						previousUtilizationOfMem * 100,
						utilizationOfMem * 100);
				Log.formatLine(
	...
	public static double getFramePower(){
		return tmp;
	}
