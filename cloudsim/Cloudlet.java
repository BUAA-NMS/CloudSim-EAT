...
public class Cloudlet {
	 ...
	 * ignored.
	 * 存储最新的执行开始时间。如果有CANCEL, PAUSED和RESUMED等行为，就纪录最新的时间。
	 */
	private double execStartTime;

	/** The ID of a reservation made for this cloudlet. 保留id －1为不保留*/
	private int reservationId = -1;

	/** 
	 * The records the transaction history for this Cloudlet. 
	 * 标记是否纪录cloudletd的交换历史（如创建cloudlet）
	 * */
	...
	/** The res list. 执行该任务的资源纪录，最新的纪录id由index确定*/
	...
	private static class Resource {
		...
		/** Cloudlet's length finished so far. 任务至今已经完成的部分*/
	 ...
	 * cancel it.
	 * 返回任务已经执行完成的长度，如果该任务已经完成，则返回任务的长度
	 * 
	 ...
	 * 设定该任务已经执行的长度。这个方法有Rescloudlet调用。当该任务被取消或者迁移到其他资源上是调用。
	 * 
	...
	public void setResourceParameter(final int resourceID, final double cost) {
		...
		// 任务放置到了一个新的云资源上面，则resList中添加这个资源
		resList.add(res);
	...
	/**
	 * Gets the utilization of memory in percentage.
	 * 
	 * @param time the time
