...
public class DatacenterCharacteristics {
	...
	/** Price/CPU-unit if unit = sec., then G$/CPU-sec. 为CPU定价*/
	private double costPerSecond;

	/** Resource Types -- allocation policy. 资源allocation原则*/
	private int allocationPolicy;

	/** Time-shared system using Round-Robin algorithm. 轮询调度*/
	public static final int TIME_SHARED = 0;

	/** Spaced-shared system using First Come First Serve (FCFS) algorithm. 先到先服务原则*/
	 ...
	 * 同一主机中的PE有相同的速度，但是主机不同速度速度不定
	 */
	public static final int OTHER_POLICY_DIFFERENT_RATING = 3;

	/** A resource that supports Advanced Reservation mechanisms. 高阶保留机制*/
