...
import org.cloudbus.cloudsim.examples.fm.mainClass;
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			...
			vms.add(new PowerVm(
					...
					1,
					"Xen",
					new CloudletSchedulerDynamicWorkload(Constants.VM_MIPS[vmType], Constants.VM_PES[vmType], Constants.VM_RAM[vmType]),
...
//		double energy = datacenter.getPower();
		...
		if (outputInCsv) {
			...
			Log.printLine();
//			Log.printLine(String.format("Experiment name: " + experimentName + "_"+Constants.Order));
			...
			if(Constants.LIMIT_VM_NUM)
			{
				Log.printLine(String.format("Limit of VMs per Host: " + Constants.HOST_VM_MAX_NUM));
			}
			else
			{
				Log.printLine(String.format("No limit"));
			}
			...
			try {
				pw1 = new PrintWriter(new FileWriter("D://test//C1//"+experimentName+".txt",true),true);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			int t = (int) energy;
			pw1.println(Integer.toString((t)));
			
			try {
				pw2 = new PrintWriter(new FileWriter("D://test//C2//"+experimentName+".txt",true),true);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			pw2.println(Integer.toString(numberOfMigrations));
			
			//String ttt = "E://graduate_program//cloud3//cloud3//src//workload//"+Constants.workload_path;
			//getFile(ttt,experimentName);
